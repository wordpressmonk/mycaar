...
(function ($) {
  var FormBuilder = function FormBuilder(options, element) {
    ...
    var frmbFields = [{
      ...
      label: opts.messages.image,
      attrs: {
        type: 'img',
        className: 'img-responsive',
      }
    }, {
      label: opts.messages.video,
      attrs: {
        type: 'video',
        className: 'file-input',
      }
    }, {
      label: opts.messages.audio,
      attrs: {
        type: 'audio',
        className: 'file-input',
      }
	...
	, {
      label: opts.messages.textdisplay,
      attrs: {
        type: 'textdisplay',
        className: 'paragraph',
      }
    ...
    if (opts.showActionButtons) {
      ...
      //$stageWrap.prepend(formActions);
    }

    $stageWrap.append($sortableFields, cbWrap);
    $formWrap.before($stageWrap);
    $formWrap.append(cbWrap, $stageWrap,formActions);
    ...
    var advFields = function advFields(values) { 
      var advFields = [],
        ...
        return optionFields.indexOf(values.type) !== -1;
          ...
          valueField = !utils.inArray(values.type, ['header', 'paragraph', 'file','textdisplay','fileupload'].concat(optionFields)),
...
/* 		if(values.type === 'img' || values.type === 'video' || values.type === 'audio') {
      ...
      if (!noMakeAttr.some(function (elem) {
        ...
        if (attribute === 'label' && utils.inArray(values.type, textArea) || attribute === 'value' && values.type === 'textarea') {
		 ...
		 /** Image **/
			...
			if(values.src)
				val = values.src;
          attributefield += '<input onChange="saveFile(this);return false;" src="'+values.src+'" type="file" '+ utils.attrString(inputConfig) +'  data_mc_type="image"  accept="image/*" style="width:18%;display:inline-block"><input  data_mc_type="image"  placeholder="Or paste URL here" type="text" class="form-control url_field" value="'+val+'"  onChange="saveUrl(this);return false" style="width:80%;display:inline-block;margin-left:5px">';
		  if(values.src)
		...
		/** Audio **/
			...
			if(values.src)
				val = values.src;
          attributefield += '<input onChange="saveFile(this);return false;" src="'+values.src+'" type="file" '+ utils.attrString(inputConfig) +' data_mc_type="audio"  accept="audio/*" style="width:18%;display:inline-block"><input data_mc_type="audio" placeholder="Or paste URL here" type="text" class="form-control url_field" value="'+val+'" onChange="saveUrl(this);return false" style="width:80%;display:inline-block;margin-left:5px">';
		  if(values.src)
		...
		/** File Download **/
			...
			if(values.src)
				val = values.src;		
          attributefield += '<input onChange="saveFile(this);return false;" src="'+values.src+'" type="file" '+ utils.attrString(inputConfig) +' data_mc_type="file"  style="width:18%;display:inline-block"><input data_mc_type="file" placeholder="Or paste URL here" type="text" class="form-control url_field" value="'+val+'" onChange="saveUrl(this);return false" style="width:80%;display:inline-block;margin-left:5px">';
		  if(values.src)
    ...
    //  var liContents = utils.markup('div', [toggleBtn, copyBtn, delBtn], { className: 'field-actions' }).outerHTML;
     var liContents = utils.markup('div', [toggleBtn, delBtn], { className: 'field-actions' }).outerHTML;
	  ...
	  liContents += '<span class="field_type_display">'+label+'</span>';
    ...
    $sortableFields.on('click touchstart', '.delete-confirm', function (e) {
      ...
      var buttonPosition = this.getBoundingClientRect(),
          ...
          coords = {
        pageX: (buttonPosition.left + buttonPosition.width) / 1.1,
